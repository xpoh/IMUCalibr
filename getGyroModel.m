function [res, f, m] = getGyroModel( data )
    N=10;
    N1=10;
...
%     m.m = -[[-0.3, -0.5, 0.8]; [-0.8, 0.65, 0.1]; [0.56, 0.56, 0.56]];
    ...
    wy=data{6,1};  % Поворот вокруг y
    wz=data{8,1};  % Поворот вокруг z
    
    m.T = mean(data{1,1}.period);
    for j=1:N
       fprintf("Step %d:\n", j); 
       m=updateArray(w0x,w0y,w0z,wx,wy,wz,m);
...
res=[m.k,m.z,m.m(1,:),m.m(2,:),m.m(3,:)];
[m.q, m.r] = qr(m.m);

%  Минусы для работы в системе СТНО
